...
//
// 🛑 MATRICE DE TRANSPORTS — la MEME suite tourne sur stdio ET sur http (le mode de la PROD).
// Avant le 02/08 elle ne couvrait que stdio : tout le pipeline HTTP — daemon, gardien, sessions
// Streamable HTTP, flux GET serveur->client — n'etait exerce bout-en-bout par AUCUN test, alors
// que c'est CE chemin que les agents empruntent reellement.
// ⚠️ ZERO ASSERTION DUPLIQUEE : c'est la MEME liste de `test()`, parametree. Dupliquer la suite
// l'aurait fait diverger, et une divergence entre transports serait devenue invisible — soit
// exactement le trou qu'on ferme ici. Le faux backend, lui aussi, sert les deux transports depuis
// un unique `handle()` (cf fixtures/fake-backend.js).
...
describe.each([
  { nom: 'stdio', http: false }, // child MCP prive au proxy (defaut, tests)
  { nom: 'http', http: true },   // serveur partage via DAEMON + GARDIEN = LE CHEMIN DE PROD
])('transport $nom', ({ nom, http }) => {
  let proc;
  let cfgPath;
  let buf = '';
  ...
  function baseCfg() {
    return {
      // ⚠️ LE SEUL PARAMETRE QUI CHANGE ENTRE LES DEUX PASSES. Tout le reste — profils, backend,
      // assertions — est strictement identique : c'est ce qui rend la comparaison valable.
      http,
  ...
  beforeAll(async () => {
    cfgPath = path.join(os.tmpdir(), `pw-mcp-test-${process.pid}-${nom}.json`);
    writeCfg(baseCfg());
    proc = spawnTracked([INDEX], {
      env: { ...process.env, PW_MCP_PROFILES: cfgPath, PW_MCP_LOG: path.join(os.tmpdir(), `pw-mcp-test-${process.pid}-${nom}.log`) },
